...
def load_tweets_from_csv(csv_path: str) -> pd.DataFrame:
    """
    Step 1.1: Data Ingestion using Pandas.
    Reads a CSV archive file into a pandas DataFrame using string dtype for IDs
    to avoid precision loss or floating-point conversions.
    """
...
def filter_applesupport_tweets(
    ...
    """
    Step 1.2: Data Filtering using Pandas & NumPy.
...
def reconstruct_user_brand_pairs(
    ...
    """
    Step 2: Conversation Thread Reconstruction.
...
def upsert_records_to_pinecone(
    ...
    """
    Step 3: Pinecone DB Vector Database Insertion.
    ...
    if not api_key:
        raise ValueError("PINECONE_API_KEY environment variable is not set in .env")
...
def process_archive_pipeline(
    csv_path: str, brand: str = "AppleSupport", limit: Optional[int] = None
) -> List[Dict[str, Any]]:
    """
    Full Data Ingestion, Filtering, and Thread Reconstruction Pipeline.
    """
    print(f"📂 [Step 1.1 - Ingestion] Reading CSV dataset: {csv_path}...")
    df = load_tweets_from_csv(csv_path)
    print(f"✅ Loaded {len(df):,} raw tweets.")

    print(f"🔍 [Step 1.2 - Filtering] Filtering tweets for brand '{brand}'...")
    filtered_df = filter_applesupport_tweets(df, target_brand=brand)
    print(f"✅ Isolated {len(filtered_df):,} '{brand}' related tweets.")

    print(f"🔄 [Step 2 - Thread Reconstruction] Reconstructing Customer -> Brand resolution pairs...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Data Ingestion, Filtering, Thread Reconstruction, and Pinecone DB Insertion for AppleSupport chats."
    )
    parser.add_argument(
        "--file",
        type=str,
        default="archive/twcs/twcs.csv",
        help="Path to archive CSV file (default: archive/twcs/twcs.csv)",
    )
    parser.add_argument(
        "--brand",
        type=str,
        default="AppleSupport",
        help="Brand name to filter and reconstruct for (default: AppleSupport)",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
        help="Optional limit on maximum records to process/upsert",
    )
    parser.add_argument(
        "--limit-preview",
        type=int,
        default=2,
        help="Number of reconstructed records to preview (default: 2)",
    )
    parser.add_argument(
        "--upsert",
        action="store_true",
        help="Perform insertion into Pinecone Vector DB (default: False)",
    )
    parser.add_argument(
        "--upsert-limit",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=96,
        help="Batch size for Pinecone upsert (max: 96, default: 96)",
    )
    parser.add_argument(
        "--namespace",
        type=str,
    ...
    records = process_archive_pipeline(args.file, brand=args.brand, limit=args.limit)
    print(f"Total number of threads extracted: {len(records):,}")

    print(f"\n--- 📋 Preview of Reconstructed Records (Showing first {args.limit_preview}) ---")
    for i, record in enumerate(records[: args.limit_preview]):
        print(f"\n==================== [Record #{i+1}] ====================")
    ...
    if args.upsert and records:
        ...
        if args.upsert_limit and args.upsert_limit > 0:
            records_to_upsert = records[: args.upsert_limit]
